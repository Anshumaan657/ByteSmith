...
export class SQLiteStore {
  ...
  async getManifest(manifestId: string): Promise<ImpactManifest | undefined> {
    ...
    try {
      ...
      if (
        ...
        throw new Error("Stored manifest digest does not match.");
      }
      await assertImpactManifest(manifest);
      return manifest;
    } catch (cause) {
      throw new StorageError(
        "manifest_corrupt",
        `Stored manifest ${manifestId} is corrupt.`,
        { cause },
      );
    }
  }

  listRuns(limit = 20): StoredRunSummary[] {
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 1000) {
      throw new RangeError("Run history limit must be between 1 and 1000.");
    }
    return (
      this.database
        .prepare(
          `SELECT manifest_id, repository_id, base_revision, head_revision,
                  merge_base_revision, configuration_digest, semantic_digest, created_at
           FROM bytesmith_runs ORDER BY created_at DESC, manifest_id ASC LIMIT ?`,
        )
      ...
      if (!row) {
        throw new StorageError(
          "manifest_corrupt",
          "Stored run history contains an invalid row.",
        );
      }
      return {
        manifestId: textColumn(row, "manifest_id"),
        repositoryId: textColumn(row, "repository_id"),
        baseRevision: textColumn(row, "base_revision"),
        headRevision: textColumn(row, "head_revision"),
        mergeBaseRevision: textColumn(row, "merge_base_revision"),
        configurationDigest: textColumn(row, "configuration_digest"),
        semanticDigest: textColumn(row, "semantic_digest"),
        createdAt: textColumn(row, "created_at"),
      };
    });
  }

  invalidateCache(identity: CacheIdentity): boolean {
    const key = createCacheKey(identity);
