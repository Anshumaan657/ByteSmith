...
import {
  ...
  recommendTests,
  normalizeTerminalTestConsumerImpacts,
...
async function sharedTestAndConsumerAnalysis(
  ...
  let typescriptConsumers: ConsumerAnalysisResult | undefined;
  let openapiConsumers: ConsumerAnalysisResult | undefined;
  if (families.typescript && families.typescript.result.snapshot) {
    typescriptConsumers = analyzeTypeScriptConsumers({
  ...
  if (families.openapi && families.openapi.result) {
    openapiConsumers = await analyzeOpenApiConsumers({
  ...
  const discovery = config.analyzers.tests.enabled
    ? await runTestDiscoveryComparison({
        repositoryId: comparison.identity.id,
        base: { directory: snapshots.base, revision: comparison.mergeBase },
        head: { directory: snapshots.head, revision: comparison.head.commit },
    ...
    : undefined;
  if (typescriptConsumers && discovery && families.typescript) {
    typescriptConsumers = normalizeTerminalTestConsumerImpacts(
      families.typescript.result.ir,
      typescriptConsumers,
      discovery,
    );
  }
  const consumerResults = [typescriptConsumers, openapiConsumers].filter(
    (value): value is ConsumerAnalysisResult => value !== undefined,
  );
  ...
  checkAborted(signal);
  if (!discovery) {
    throw new TypeError("Test discovery was not initialized.");
  }
