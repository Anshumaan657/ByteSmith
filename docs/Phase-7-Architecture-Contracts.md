...
- `verify-impact --manifest FILE`; and
- `benchmark [--case ID] [--repeat NUMBER]`.
...
`verify-impact` validates the manifest schema, semantic rules, and digest before
checking its repository identity and both exact commits. It additionally
requires the repository's current `HEAD` to equal the manifest head. A later
commit therefore makes an otherwise valid result stale instead of silently
reusing it.
